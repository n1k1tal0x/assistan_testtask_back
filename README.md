...
- `POST /requests` (`src/routes/requests.ts`) — создание заявки. Валидация: `fullName` и `reason`
  ...
  не может быть раньше `dateFrom`. При ошибке — `400` с описанием, при успехе — `201` и
  созданная заявка с сгенерированным `id` и статусом `pending`. Количество дней отпуска
...
`npm test` — тесты на встроенном тест-раннере Node.js (`node:test`) через `tsx`, без реального
сетевого порта (`app.inject()` из Fastify). Покрыты `POST /requests` (успешное создание,
валидация: короткая причина, дата в прошлом, `dateTo` раньше `dateFrom`), `GET /requests`
(пароль обязателен/неверен/верен, фильтр по статусу) и `PATCH .../approve` + `.../reject`
(успешное решение, повторное решение — `409`, несуществующий `id` — `404`, отказ без причины —
`400`, причина отказа видна после отклонения). `src/app.ts` собирает Fastify-инстанс отдельно от
`src/index.ts`, чтобы тесты могли переиспользовать его без `listen()`.
